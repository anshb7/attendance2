...
class GoogleSheetsApi {
  ...
  Future init() async {
    ...
    _worksheet = ss.worksheetByTitle('Attendance2');
  }

  // static Future<List<User>> getAll() async {
  //   if (_worksheet == null) return <User>[];
  //
  // users == null ? <User>[] : users.map(User.fromJson).toList();
  // }

  static Future insert(User? user, String teamno) async {
    if (_worksheet == null) return;
    // final users = await _worksheet!.values.map.allRows();
    // final finalusers = users!.map(User.fromJson).toList();
    // print(finalusers);
    if (user == null) {
      return;
    } else {
